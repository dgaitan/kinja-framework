...
class MakeActionCommand extends Command
{
    ...
    protected function configure()
    {
        ...
        $this->setDescription( 'Generate an action for a Feature' );
    ...
    protected function execute( InputInterface $input, OutputInterface $output ) {
        ...
        if ( ! $name ) {
            $output->writeln('<error>Action name is required</error>');
        ...
        try {
            ...
            $output->writeln(sprintf('<info>Action %s was created in %s</info>', $name, $feature));
        } catch ( Throwable $e ) {
    ...
    protected function get_namespace( string $feature ) : string {        
        ...
        return sprintf( '%s\Features\%s\Actions', KINJA_PLUGIN_NAMESPACE, $feature );
    }
    ...
    public function get_file( string $name, string $feature ) : string {
        ...
        $file = sprintf( '%s/Features/%s/Actions/%s', KINJA_APP_PATH, $feature, $name );
    ...
    protected function maybe_create_dir( string $feature ) {
        ...
        $dir = sprintf( '%s/Features/%s/Actions', KINJA_APP_PATH, $feature );
